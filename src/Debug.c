...
byte DebugZ80(Z80 *R)
{
  ...
  printf
  (
    "AF:%04X HL:%04X DE:%04X BC:%04X IX:%04X IY:%04X I:%02X\n",
    R->AF.W,R->HL.W,R->DE.W,R->BC.W,R->IX.W,R->IY.W,R->I
  ); 
  printf
  ( 
    "%04X[%02X - %s] SP:%04X[%04X] FLAGS:[%s] %s:%s\n",
    R->PC.W, RdZ80(R->PC.W),S, R->SP.W, RdZ80(R->SP.W)+RdZ80(R->SP.W+1)*256,T,
